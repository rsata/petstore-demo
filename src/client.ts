...
export interface ClientOptions {
  /**
   * Defaults to process.env['PETSTORE_DEMO_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['PETSTORE_DEMO_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['PETSTORE_DEMO_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Petstore Demo API.
 */
export class PetstoreDemo {
   ...
   * API Client for interfacing with the Petstore Demo API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['PETSTORE_DEMO_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['PETSTORE_DEMO_BASE_URL'] ?? /api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('PETSTORE_DEMO_BASE_URL'),
    apiKey = readEnv('PETSTORE_DEMO_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.PetstoreDemoError(
        "The PETSTORE_DEMO_API_KEY environment variable is missing or empty; either provide it, or instantiate the PetstoreDemo client with an apiKey option, like new PetstoreDemo({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? PetstoreDemo.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('PETSTORE_DEMO_LOG'), "process.env['PETSTORE_DEMO_LOG']", this) ??
      defaultLogLevel;
  ...
  static PetstoreDemo = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static PetstoreDemoError = Errors.PetstoreDemoError;
